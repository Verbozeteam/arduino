...
if [ "$NO_LIBS" = "1" ]; then
    ...
    GCC_ARGS=' -D _COMPILE_NO_LIBS_'
fi

SOURCES="../$BOARD/$BOARD.ino $LIBRARY_SOURCES $STUB_SOURCES"
EXTRA_INCLUDES="$LIBRARY_INCLUDE_DIRS $STUB_INCLUDE_DIRS"
python $PATH_TO_SHAMMAM -n $BOARD -s $SOURCES -i $EXTRA_INCLUDES -p $PROTOCOL --port $RPC_PORT --serial_port $SERIAL_PORT --time $TIME_MULTIPLIER --debug --gcc "$GCC_ARGS"
